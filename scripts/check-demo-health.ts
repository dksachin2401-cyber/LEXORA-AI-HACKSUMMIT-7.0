...
const EXPRESS_BASE_URL = process.env.VITE_API_BASE_URL ? process.env.VITE_API_BASE_URL.replace('/api', '') : 'http://localhost:5000';
...
async function runDemoHealthCheck() {
  ...
  try {
    const r = await fetch(`${EXPRESS_BASE_URL}/api/health`);
    results.push({
    ...
    results.push({
      component: 'Express Gateway (/api/health)',
      pass: false,
      details: `Express server offline at ${EXPRESS_BASE_URL}`,
    });
  }
  ...
  try {
    ...
    results.push({
      component: 'FastAPI AI Engine (/health)',
      pass: false,
      details: `FastAPI server offline at ${FASTAPI_BASE_URL}`,
    });
  }
  ...
  if (allPass) {
    ...
    console.log('DEMO HEALTH CHECK: FAIL (Inspect errors above)');
  }
  console.log('==================================================\n');
